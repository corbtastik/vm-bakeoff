# VM Bakeoff: Lima (VZ) + Ubuntu (Apple Silicon) + Per-VM Provisioning 🧪🍏

This repo is a **VM bakeoff framework** for running Linux VMs on **Apple Silicon macOS** using **Lima + Virtualization.framework (VZ)** — with a clean separation between:
...
- No “one VM running everything”
- Manual, per-VM “offset style” host port forwards to avoid collisions

> Blog Post (original): VMs on macOS (Apple Silicon) with Lima — https://corbs.io/
...
## VM configuration (single place) ⚙️
...
## Software configuration (separate place) 🧩
...
* Pinned image digest (`platforms/lima/images/ubuntu.env`) can be tracked or regenerated — your choice
